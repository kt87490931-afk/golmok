...
function dedupeFooters() {
  const footers = [...document.querySelectorAll('.gm-footer, #site-footer')];
...
async function mountSiteFooterInner(ctx) {
  ...
  if (existing?.dataset.mounted === '1') return;
  ...
  if (!footer) {
    try {
      ...
      if (!footer) footer = insertFooterElement(html);
    } catch (err) {
...
export async function mountSiteFooter(ctx = detectContext()) {
  ...
  _mountPromise = mountSiteFooterInner(ctx).finally(() => {
    _mountPromise = null;
  });
