import { getCurrentUser, getUserProfile, getDongPosts, getSigunguPosts, getAllPosts, getEventPosts } from '../community.js';
...
function renderEventCards(events) {
  ...
  if (!grid) return;
  if (!events.length) {
    grid.innerHTML = '<div style="padding:16px;color:#999;font-size:13px;">등록된 이벤트·이슈가 없습니다.</div>';
    return;
  }
  ...
  if (!user) {
    wrap.innerHTML = `
      <div style="text-align:center;padding:60px 20px;background:#fff;border-radius:12px;border:1px solid #E8E4DC;">
        ...
        <div style="font-size:14px;color:#999;margin-bottom:20px;">우리동네 피드는 로그인 후 이용할 수 있어요</div>
  ...
  if (!userProfile?.region_dong) {
    wrap.innerHTML = `
      <div style="text-align:center;padding:60px 20px;background:#fff;border-radius:12px;border:1px solid #E8E4DC;">
        ...
        <div style="font-size:14px;color:#999;margin-bottom:20px;">우리동네 피드를 보려면 활동 지역을 설정해야 해요</div>
